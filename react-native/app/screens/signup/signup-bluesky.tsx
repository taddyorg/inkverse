...
import { SIGNUP_COMPLETE_SCREEN, RootStackParamList } from '@/constants/Navigation';
...
export function SignupBlueskyScreen() {
  ...
  const handleSkip = useCallback(() => {
    navigation.navigate(SIGNUP_COMPLETE_SCREEN);
  }, [navigation]);
  ...
  const handleBlueskyVerify = useCallback(async () => {
    ...
    if (!isValidDomain(trimmedHandle)) {
      dispatch({ 
        ...
        payload: 'Invalid Bluesky handle. Make sure you use your handle (ex: bsky.app/profile/yourhandle)' 
      });
      return;
    }
